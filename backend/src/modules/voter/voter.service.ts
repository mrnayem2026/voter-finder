...
export class VoterService {
        ...
        try {
            ...
            if (error.code === "P2002") {
                ...
                throw ApiError.conflict(
                    `A voter with this ${target.join(", ")} already exists`
                );
            }
            throw error;
        }
    ...
    static async search(query: SearchVoterQuery) {
        console.log("🔍 Search query received:", JSON.stringify(query));
        const { search = "", page = 1, limit = 20 } = query;
        console.log(`📊 Parsed: search="${search}", page=${page}, limit=${limit}`);

        const skip = (page - 1) * limit;

        const where: VoterWhereInput = search
            ? {
                voterName: {
                    contains: search,
                    mode: "insensitive",
    ...
    static async importCsv(fileBuffer: Buffer) {
        ...
        const mapRecord = (record: Record<string, string>) => ({
            voterSlipNumber:
                record["Voter Slip Number"] ||
                record["voterSlipNumber"] ||
                record["voter_slip_number"] ||
                "",
            voterName:
                record["Voter Name"] ||
                record["voterName"] ||
        ...
        if (newRecords.length > 0) {
            await prisma.$transaction(
                newRecords.map((record) => prisma.voter.create({ data: record }))
            );
